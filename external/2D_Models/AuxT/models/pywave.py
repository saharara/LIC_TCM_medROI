...
class DWT_Function(Function):
    ...
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):
        ...
        x_ll = torch.nn.functional.conv2d(x, w_ll.expand(dim, -1, -1, -1), stride = 2, groups = dim,padding=w_ll.shape[-1]//4)
        x_lh = torch.nn.functional.conv2d(x, w_lh.expand(dim, -1, -1, -1), stride = 2, groups = dim,padding=w_ll.shape[-1]//4)
        x_hl = torch.nn.functional.conv2d(x, w_hl.expand(dim, -1, -1, -1), stride = 2, groups = dim,padding=w_ll.shape[-1]//4)
        x_hh = torch.nn.functional.conv2d(x, w_hh.expand(dim, -1, -1, -1), stride = 2, groups = dim,padding=w_ll.shape[-1]//4)
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            dx = torch.nn.functional.conv_transpose2d(dx, filters, stride=2, groups=C,padding=w_ll.shape[-1]//4)
